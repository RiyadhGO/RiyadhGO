 ...
 */
public class BusMethodFragment extends Fragment
        implements DataArrivedListener,PlaceSelectedListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View root = inflater.inflate(R.layout.fragment_bus_method, container, false);
        dataRV = root.findViewById(R.id.bus_meth_fgmt_transport_method_rv);
        navView = root.findViewById(R.id.bus_nav_view);
    ...
    private void getTypeMethods(){
        getActivity().runOnUiThread(new Runnable() {
            @Override
            public void run() {
                LocationClient.findLocation(TransportMethodTypes.BUS.toString(),
                        BusMethodFragment.this, DataArrivedListener.RC_FIND_BUS_LOCATIONS);
            }
        });
    }
            ...
            public void run() {
                ...
                ScooterBusLocationsRVAdapter adapter =
                        new ScooterBusLocationsRVAdapter( models,
                                BusMethodFragment.this,StartFragment.pickLocation.getLatLng());
    ...
    private void setNavViewClickListener() {
        navView.setSelectedItemId(R.id.navigation_bus);
        navView.setOnItemSelectedListener(new NavigationBarView.OnItemSelectedListener() {
            @Override
            public boolean onNavigationItemSelected(@NonNull MenuItem item) {
                if (item.getItemId() == R.id.navigation_bus) {
                } else if (item.getItemId()  == R.id.navigation_walking) {
                    navigateToSubAction(R.id.action_busMethodFragment_to_walkingMethodFragment);
